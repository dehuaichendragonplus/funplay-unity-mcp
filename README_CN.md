...
- **95 个内置工具** — 覆盖场景编辑、脚本、资产、运行态控制、截图、性能分析、Prompts、Resources、结构化对象定位、SerializedObject 组件编辑、编辑器状态读写、菜单项兜底以及编辑器自动化，共 21 个模块
...
| 工具暴露 | 95 工具 / 21 模块，`core` (30) / `full` profile | ~15 个 MCP 工具（多数为 `Manage*` 大粒度族）|
...
Funplay MCP for Unity 当前提供 **95 个工具函数**，覆盖 21 个模块：
    ...
    └─ MCPRequestHandler (协议处理)
        └─ MCPExecutionBridge
            └─ FunctionInvokerController (反射式调用)
                └─ Tool Functions (95 个内置工具，21 个模块)
```
