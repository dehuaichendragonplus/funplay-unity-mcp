...
Describe your game in one sentence — your AI assistant builds it in Unity through Funplay MCP for Unity's 108 built-in tools for scene creation, script generation, runtime validation, input simulation, performance analysis, and editor automation.
...
- **Focused by Default, Full When Needed** — `core` exposes a compact high-signal toolset; `full` exposes all 108 tools
...
- **108 Built-in Tools** — Scene editing, assets, scripts, play mode control, screenshots, performance analysis, prompts, resources, structured object location, SerializedObject-based component editing, editor-state inspection, menu-item fallback, and editor automation across 22 modules
...
| Tool exposure | 108 tools across 22 modules, `core` (30) / `full` profiles | ~15 MCP tools (mostly `Manage*` families) |
...
- **Tools** — 108 total tools in `full`, 30 focused tools in `core`
...
Funplay MCP for Unity currently ships with **108 tool functions** across 22 modules:
    ...
    └─ MCPRequestHandler (protocol handling)
        └─ MCPExecutionBridge
            └─ FunctionInvokerController (reflection-based invocation)
                └─ Tool Functions (108 built-in tools across 22 modules)
```
